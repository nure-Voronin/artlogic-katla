...
export class HiveSectionService {
  ...
  updateHiveSection(
    ...
    StoreHiveId: number
  ): Observable<HiveSection> {
    return this.http.put<HiveSection>(`${this.url}${hiveSection.id}`, {
      ...hiveSection,
      StoreHiveId,
    });
  }
  ...
  setHiveSectionStatus(
    ...
    return this.http.put<HiveSection>(
      `${this.url}${hiveSectionId}/status/${deletedStatus}`,
      ""
    );
  }
}
