...
export class HiveFormComponent implements OnInit {
  ...
  onDelete() {
    this.hiveService
      .setHiveStatus(this.hive.id, true)
      .subscribe((c) => (this.hive.isDeleted = true));
  }

  onUndelete() {
    this.hiveService
      .setHiveStatus(this.hive.id, false)
      .subscribe((c) => (this.hive.isDeleted = false));
  }

  onPurge() {
    this.hiveService
      .deleteHive(this.hive.id)
      .subscribe((c) => this.navigateToHives());
  }
}
